...
def parse_table_data(raw_text: str, reporting_periods: List[str]) -> List[IncomeStatementLineItem]:
    ...
    for i, row in enumerate(table_rows):
        # Clean up the row data (handle variable column counts)
        account_name = row[0].strip()
        val1 = clean_value(row[1]) if len(row) > 1 else ""
        val2 = clean_value(row[2]) if len(row) > 2 else ""
        ...
        if is_section_header:
            ...
            logger.debug(f"  OK: Section header detected - setting context to '{current_section}'")
            ...
            logger.debug(f"  OK: Section header added to output")
        ...
        if is_total_line_that_resets_context(account_name):
            ...
            logger.debug(f"  INFO: Total line detected - resetting section context")
        ...
        if values:  # Only add if we have values
            ...
            logger.debug(f"  WARNING: No values found, skipping")
