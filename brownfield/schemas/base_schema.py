...
class ExcelColumnMapping(BaseModel):
    ...
    sub_header: Optional[str] = Field(description="Sub-header text", default="")
    span_columns: int = Field(description="How many Excel columns this header spans", default=1)
    data_type: str = Field(description="Data type for formatting (text, currency, number, percentage)", default="text")
    merge_with_next: bool = Field(description="Whether to merge with next column for main header", default=False)
...
class ExcelLayoutConfig(BaseModel):
    ...
    column_mappings: List[ExcelColumnMapping] = Field(description="Column definitions and formatting")
    has_multi_level_headers: bool = Field(description="Whether this has main + sub header rows", default=False)
...
class BaseFinancialSchema(BaseModel):
    ...
    units_note: Optional[str] = Field(description="Units note (e.g., 'In millions, except per share data')", default="")
    consolidation_summary: Optional[Dict[str, Any]] = Field(description="Summary of account consolidations for transparency", default=None)
...
class BaseLineItem(BaseModel):
    ...
    indent_level: int = Field(description="Indentation level for hierarchy (0=main level)", default=0)
    is_section_header: bool = Field(description="Whether this is a section header", default=False)
    parent_section: Optional[str] = Field(description="Name of parent section if applicable", default="")
...
class HierarchicalLineItem(BaseLineItem):
    ...
    values: Dict[str, str] = Field(description="Values for each reporting period")
    sub_items: Optional[List["HierarchicalLineItem"]] = Field(description="Sub-items under this account", default=[])
...
class ColumnHeader(BaseModel):
    ...
    sub_header: Optional[str] = Field(description="Sub-header under the main header", default="")
