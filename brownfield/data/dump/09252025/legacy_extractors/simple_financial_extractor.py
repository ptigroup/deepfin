...
def check_cached_result(whisper_hash):
...
def save_cached_result(whisper_hash, extracted_text):
...
def quick_text_extraction_for_detection(pdf_path):
    ...
    try:
        ...
        if status_code == 200 and "extracted_text" in result:
            ...
            print(f"🔍 Debug: Raw whisper_hash from result: '{whisper_hash}'")
            print(f"🔍 Debug: Full result keys: {list(result.keys())}")
            if whisper_hash:
                full_hash = str(whisper_hash).strip()
                
                # Check cache first
                cached_result = check_cached_result(full_hash)
                if cached_result:
                    return cached_result[:2000]  # First 2000 chars for detection
                ...
                for attempt in range(3):
                    time.sleep(3)
                    try:
                        status_result = llmw.whisper_status(whisper_hash=full_hash)
                        if status_result.get('status') == 'processed':
                            retrieve_result = llmw.whisper_retrieve(whisper_hash=full_hash)
                            extracted_text = retrieve_result.get('extracted_text', '')
                            if extracted_text:
                                # Cache the result
                                save_cached_result(full_hash, extracted_text)
                                return extracted_text[:2000]  # First 2000 chars for detection
                    except Exception as e:
                        error_msg = str(e).lower()
                        if 'already retrieved' in error_msg:
                            print(f"⚠️ Detection hash already retrieved: {full_hash}")
                            # Check cache one more time
                            cached_result = check_cached_result(full_hash)
                            if cached_result:
                                return cached_result[:2000]
