...
def parse_shareholders_equity_directly(raw_text_file_path):
    ...
    print(f"Found {len(data_rows)} data rows to parse")
    ...
    for line_num, line in data_rows:
        ...
        print(f"Parsing: {description[:50]}...")
        ...
        print(f"  ✅ Mapped {len(column_values)} columns")
    ...
    print(f"📊 Extracted {len(equity_rows)} rows")
    print(f"📈 Found {len([row for row in equity_rows if row['row_type'] == 'balance'])} balance rows")
    print(f"📋 Found {len([row for row in equity_rows if row['row_type'] == 'transaction'])} transaction rows")
...
def save_direct_extraction(structured_data, source_pdf_path):
    ...
    json_path = f"output/structured/json/{pdf_name}_direct_extraction.json"
    
    with open(json_path, 'w') as f:
        json.dump(output_data, f, indent=2)
    
    print(f"✅ Direct extraction JSON saved to: {json_path}")
    
    # Save Excel using existing schema system
    excel_path = f"output/structured/excel/{pdf_name}_direct_extraction.xlsx"
    ...
    try:
        ...
        print(f"✅ Direct extraction Excel saved to: {excel_path}")
        ...
        print(f"✅ Fallback Excel saved to: {excel_path}")
...
if __name__ == "__main__":
    ...
    print("🚀 Testing Direct Shareholders Equity Parser")
    print("=" * 60)
